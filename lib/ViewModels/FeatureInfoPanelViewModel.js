...
FeatureInfoPanelViewModel.prototype.getWidth = function(el) {
    var sections0 = this.sections;
    var selectedFeature = this.terria.selectedFeature;
    var width = 'auto'

    var that = this;
    var sections = [].slice.call(document.querySelectorAll('.feature-info-panel-section-inner'));
    sections.forEach(function(d, i){
        if (selectedFeature === knockout.dataFor(d).feature && sections0.indexOf(knockout.contextFor(d) >= 0)){
